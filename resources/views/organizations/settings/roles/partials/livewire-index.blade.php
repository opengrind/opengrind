<div class="md:grid md:grid-cols-3 md:gap-16">
  <div class="md:col-span-1">
    <h2 class="text-lg mb-2 font-bold">{{ __('What can users do?') }}</h2>
    <p class="text-sm">{{ __('Roles are used to grant access and permissions for users. In addition to the available pre-defined roles, you can create as many custom roles as needed to fit your needs.') }}</p>
  </div>

  <div class=" md:mt-0 md:col-span-2">
    @if (! $openModal)
    <div class="flex justify-end mb-3">
      <x-button-form wire:click="toggle" dusk="open-modal-button">
        {{ __('Add') }}
      </x-button-form>
    </div>
    @endif
    ...
    <form wire:submit.prevent="store" class="bg-form mb-6 rounded-lg border border-gray-200 dark:border-gray-700 dark:bg-gray-900">
      <div class="border-b border-gray-200 dark:border-gray-700 p-5">
        ...
        <div class="w-full max-w-lg">
          <x-input-label for="roleLabel" :value="__('Label')" />
          <x-text-input wire:model.defer="roleLabel" wire:keydown.escape="toggle()" id="roleLabel" class="block mt-1 w-full name" type="text" name="roleLabel" :value="old('roleLabel')" autofocus required />
      ...
      <div class="flex justify-between p-5">
        <x-button-form wire:click="toggle">{{ __('Cancel') }}</x-button-form>

        <x-button-form wire:loading.attr="disabled" primary>
          {{ __('Save') }}
        </x-button-form>
      </div>
    </form>
    ...
    <ul dusk="role-list" class="list mb-2 rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      ...
      <li :wire:key="$role['id']" class="item-list border-b border-gray-200 hover:bg-slate-50 dark:border-gray-700 dark:bg-slate-900 hover:dark:bg-slate-800">
        ...
        <form wire:submit.prevent="update({{ $role['id'] }})" class="">
          ...
          <!-- actions -->
          <div class="flex justify-between p-5">
            <x-button-form wire:click="toggleEdit(0)">{{ __('Cancel') }}</x-button-form>
